...
const Web3FunctionLoader_1 = require("./Web3FunctionLoader");
describe("Web3FunctionLoader.load", () => {
    const TEST_FOLDER_BASE = node_path_1.default.join(process.cwd(), "src/lib/loader/__test__/");
    test("should fail when source is missing", () => {
        try {
            Web3FunctionLoader_1.Web3FunctionLoader.load("source-missing", TEST_FOLDER_BASE);
    ...
    test("should load when TS source is available", () => {
        const details = Web3FunctionLoader_1.Web3FunctionLoader.load("just-ts", TEST_FOLDER_BASE);
        expect(details.path.includes("just-ts/index.ts")).toBeTruthy();
    });
    test("should load when JS source is available", () => {
        const details = Web3FunctionLoader_1.Web3FunctionLoader.load("just-js", TEST_FOLDER_BASE);
